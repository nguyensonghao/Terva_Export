...
var phInstance = null;
var BASE_URL = "http://localhost:3000/report-page4?id=57e32e1fb0ee4f26aefe916e";
	    ...
	    ph.createPage().then((page) => {
	        page.property('viewportSize', {width: 740, height: 840}).then(() => {
	            page.open(BASE_URL).then((status) => {            
	                setTimeout(function () {
	                	var imageUrl = './page4/resources/page4.png';
	...
	printPdf: function (callback) {
		capture(function (result) {
			if (result) {
				...
				var imgSrcMap = 'file://' + __dirname + '/resources/page4.png';
				...
				pdf.create(html, options).toFile('./page4/resources/page4.pdf', function(err, res) {
				  	callback(!err);
				})
			} else {
				callback(false);
