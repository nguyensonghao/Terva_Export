...
var phInstance = null;
var BASE_URL = "http://localhost:3000/report-page7?id=57e32e1fb0ee4f26aefe916e";

function capture (callback) {
	phantom.create().then((ph) => {
	    ph.createPage().then((page) => {
	        page.property('viewportSize', {width: 740, height: 840}).then(() => {
	            page.open(BASE_URL).then((status) => {            
	                setTimeout(function () {
	                	var imageUrl = './page7/resources/page7.png';
	...
	printPdf: function (callback) {
		capture(function (result) {
			if (result) {
				...
				var imgSrcMap = 'file://' + __dirname + '/resources/page7.png';
				...
				pdf.create(html, options).toFile('./page7/resources/page7.pdf', function(err, res) {
				  	callback(!err);
				})
			} else {
				callback(false);
