...
var phInstance = null;
var BASE_URL = "http://localhost:3000/report-page5?id=57e32e1fb0ee4f26aefe916e";
	    ...
	    ph.createPage().then((page) => {
	        page.property('viewportSize', {width: 740, height: 840}).then(() => {
	            page.open(BASE_URL).then((status) => {            
	                setTimeout(function () {
	                	var imageUrl = './page5/resources/page5.png';
	...
	printPdf: function (callback) {
		capture(function (result) {
			if (result) {
				...
				var imgSrcMap = 'file://' + __dirname + '/resources/page5.png';
				...
				pdf.create(html, options).toFile('./page5/resources/page5.pdf', function(err, res) {
				  	callback(!err);
				})
			} else {
				callback(false);
